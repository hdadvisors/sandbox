...
ggplot(hud_plot, aes(x = income, y = fct_rev(hh_size))) +
  ...
  geom_text(
    aes(label = label_currency(accuracy = 1)(income)),
    size = 9, color = "white", nudge_x = -3000, hjust = 1
    ) +
  geom_text(
    aes(label = label_currency(accuracy = 1)(rent), color = AMI),
    size = 9, fontface = "italic", nudge_x = 3000, hjust = 0
    ) +
  scale_x_continuous(labels = label_currency(), expand = c(0, 0), limits = c(0, 130000)) +
  theme_hda(base_size = 26) +
  labs(
    title = "HUD FY 2023 Multifamily Tax Subsidy Projects income limits",
    subtitle = "Household income limits and affordable *rents* by AMI and household size",
...
ggsave("sandbox/alx/alx-ami.png", width = 6.5, height = 3.75, units = "in", bg = "white")
...
Models where the rent subsidy is calculated based on Fair Market Rents (FMR) use the current Small Area Fair Market Rents (SAFMR) adopted by the Alexandria Redevelopment and Housing Authority for 2023. SAFMRs are provided by ZIP code.
...
fmrs_plot <- fmrs_all |> 
  ...
  mutate(
    area = str_replace_all(area, "average", "Average"),
    area = fct_other(
...
ggplot(fmrs_plot, aes(x = fmr, y = area, fill = type)) +
  ...
  geom_text(
    ...
    aes(label = label_currency(accuracy = 1)(fmr)), size = 9, color = "white", nudge_x = -500
    ) +
...
ggsave("sandbox/alx/alx-safmr.png", width = 6.5, height = 4, units = "in", bg = "white")
...
sA_subsidy_annual |> 
  mutate(across(2:4, ~formattable::digits(.x, 2))) |> 
...
sA_served |>
  select(1, 4, 5, 7, 6) |> 
  mutate(
    across(2:4, ~formattable::digits(.x, 2)),
...
sB_subsidy |>
  ...
  mutate(subsidy = formattable::digits(subsidy, 2)) |> 
